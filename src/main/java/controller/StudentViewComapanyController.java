...
public class StudentViewComapanyController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		RequestDispatcher header =
				request.getRequestDispatcher("Student/header.html");

		header.include(request, response);

		// Service Object
		ViewStudentCompanyServiceImpl service =
				new ViewStudentCompanyServiceImpl();

		ArrayList<AddCompanyModel> list =
		...
		for (AddCompanyModel company : list) {
			...
			out.println("<td class='text-center'>");
//			out.println("<button class='btn btn-success btn-sm px-4'>Apply</button>");
			out.println("<a href='applyCompany?cid="+ company.getId()
		...
		RequestDispatcher footer =
				request.getRequestDispatcher("Student/footer.html");
